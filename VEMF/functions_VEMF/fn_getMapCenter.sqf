...
if (VEMFDebugLocs) then {
	diag_log format ["[VEMF]: MISSDEBUG: Map:%1 / CenterPos:%2 / Radius:%3", _mapName, str(_centerPos), _mapRadii];
};

if ((_centerPos select 0) == 0) then {
	diag_log format ["[VEMF]: POSFinder: %1 is not a Known Map. Please Inform Vampire.", _mapName];
};
